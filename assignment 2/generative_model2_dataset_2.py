...
with open('binclassv2.txt' , 'rt') as f:
	reader  = csv.reader(f)
...
for i in range(0,x_plus.shape[0]):
	x_1 = x_plus[i] 
	sigma_optimal += np.dot((x_1-mu_plus_optimal),(x_1-mu_plus_optimal).T)

for i in range(0,x_minus.shape[0]):
	x_2 = x_minus[i]
	sigma_optimal += np.dot((x_2-mu_minus_optimal),(x_2-mu_minus_optimal).T)

sigma_optimal /= x.shape[0]
...
for point in z:
	probability1 = math.exp(((-1)*np.dot((point-mu_plus_optimal),
		(point-mu_plus_optimal).T))/(2*sigma_optimal))/(2*np.pi*np.sqrt(sigma_optimal))

	probability2 = math.exp(((-1)*np.dot((point-mu_minus_optimal),
		(point-mu_minus_optimal).T))/(2*sigma_optimal))/(2*np.pi*np.sqrt(sigma_optimal))
